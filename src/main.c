...
extern uint8_t gbc_frame_persist_reload_val;
// extern uint8_t display_off_vbl_adjust;
...
void main(void) {
    ...
    if (_cpu == CGB_TYPE) {
        ...
        while(true) {
            ...
            // Adjust the Display Persist repeat frame count
            else if (BUTTON_TOGGLED(J_LEFT)) {
                if (gbc_frame_persist_reload_val) 
                    gbc_frame_persist_reload_val--;
            }
            else if (BUTTON_TOGGLED(J_RIGHT)) {
                if (gbc_frame_persist_reload_val < 255) 
                    gbc_frame_persist_reload_val++;
            }
        }
    }
