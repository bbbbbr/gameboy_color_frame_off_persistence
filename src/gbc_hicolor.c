...
uint8_t gbc_frame_persist_count = 1;
uint8_t gbc_frame_persist_reload_val = 1;

void display_briefly_off_toggle_vbl(void) {

    if (gbc_frame_persist_count == 0) {
        // Run a normal full frame with no display persisting
        // Turn LCD isr on (for hi-color rendering)
        IE_REG |= LCD_IFLAG;

        // Prepare to persist the frame that is about to render
        // for the requested amount of frames
        gbc_frame_persist_count = gbc_frame_persist_reload_val;
    } else {
        // Turn LCD isr off (for hi-color rendering)
        IE_REG &= ~LCD_IFLAG;

        // Toggle the display briefly off/on to restart the ppu
        //
        // On GBC this will cause the previous frame to repeat since
        // the first frame after turning on the ppu is not rendered to the lcd
        // and there is some persist mechanism
        DISPLAY_OFF;
        DISPLAY_ON;
        gbc_frame_persist_count--;
    }
}
...
void hicolor_start(const hicolor_data * p_hicolor, uint8_t hicolor_bank) NONBANKED {
    ...
    CRITICAL {
        ...
        remove_VBL(display_briefly_off_toggle_vbl);
    }
    ...
    CRITICAL {
        ...
        add_VBL(display_briefly_off_toggle_vbl);
    }
